...
function Toast({ message, type, onClose }) {
  ...
  return (
    <div
      className={`fixed bottom-8 right-8 ${styles} border p-5 rounded-2xl flex items-center gap-4 z-200 animate-in fade-in slide-in-from-bottom-4 duration-300 shadow-2xl backdrop-blur-md max-w-md`}
    >
...
function TrabajadorPageContent() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (authLoading) {
      showToast(
        "Cargando información de usuario. Por favor, inténtalo de nuevo.",
        "error",
      );
      return;
    }
    ...
    if (isNaN(experienciaNum) || experienciaNum < 0) {
      showToast(
        "Los años de experiencia deben ser un número válido y positivo.",
        "error",
      );
      return;
    }
    ...
    try {
      ...
      showToast(
        "Solicitud enviada con éxito. El administrador revisará tus datos.",
      );
      ...
      showToast(
        `Hubo un error al enviar tu solicitud: ${error.message}`,
        "error",
      );
    } finally {
      setLoading(false);
  ...
  const handleCvChange = (e) => {
    ...
    if (sizeMB > MAX_CV_SIZE_MB) {
      showToast(
        `El PDF pesa ${sizeMB.toFixed(1)}MB y supera el límite de ${MAX_CV_SIZE_MB}MB.`,
        "error",
      );
      e.target.value = "";
      return;
  ...
  const handleImageChange = (e) => {
    ...
    if (oversized.length > 0) {
      const label =
        oversized.length === 1 ? "imagen supera" : "imágenes superan";
      showToast(
        `${oversized.length} ${label} el límite de ${MAX_IMAGE_SIZE_MB}MB: ${oversized.join(", ")}`,
        "error",
      );
    }
      ...
      <main className="max-w-3xl mx-auto px-6 pt-28 pb-20">
        ...
        <div className="mb-12">
          ...
          <p className="text-slate-500 text-lg font-bold">
            Completa tu perfil profesional para que el administrador valide tu
            cuenta.
          </p>
        </div>
        ...
        <form
          ...
          className="space-y-8 bg-[#111118] p-10 rounded-[3rem] border border-white/5 shadow-2xl"
        >
          <div className="space-y-6">
            ...
            <div className="space-y-3">
              <label
                htmlFor="profesion"
                className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 ml-2"
              >
              ...
              {cv ? (
                <div className="flex items-center justify-between w-full bg-[#0A0A0F] border border-white/10 rounded-2xl px-6 py-5">
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="text-[#6c63ff] shrink-0">
                      <Icons.FileText />
                    </span>
                ...
                <div className="relative">
                  ...
                  <label
                    htmlFor="cv-upload"
                    className="flex items-center justify-center w-full bg-[#0A0A0F] border-2 border-dashed border-white/10 rounded-2xl px-6 py-8 cursor-pointer hover:border-[#6c63ff]/50 transition-all group"
                  >
                    <div className="text-center flex flex-col items-center gap-2">
                      <span className="text-slate-600 group-hover:text-[#6c63ff] group-hover:scale-110 transition-all">
                        <Icons.FileText />
                      </span>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between ml-2 mr-1">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest text-slate-600">
                  Máx. {MAX_IMAGE_SIZE_MB}MB c/u · hasta {MAX_IMAGES}
                </span>
              </div>
              <input
                type="file"
              ...
              <label
                htmlFor="img-upload"
                className={`flex items-center justify-center w-full bg-[#0A0A0F] border-2 border-dashed rounded-2xl px-6 py-8 transition-all group ${imagenes.length >= MAX_IMAGES ? "border-white/5 opacity-40 cursor-not-allowed" : "border-white/10 cursor-pointer hover:border-[#6c63ff]/50"}`}
              >
                <div className="text-center flex flex-col items-center gap-2">
                  <span className="text-slate-600 group-hover:text-[#6c63ff] group-hover:scale-110 transition-all">
                    <Icons.Image />
          ...
          <button
            ...
            className="w-full bg-linear-to-r from-[#6c63ff] to-[#4b45b2] text-white py-5 rounded-3xl font-black uppercase tracking-[0.2em] text-[10px] shadow-xl shadow-[#6c63ff]/20 hover:scale-[1.02] transition-all disabled:opacity-50"
          >
            {loading
              ? "Procesando archivos..."
