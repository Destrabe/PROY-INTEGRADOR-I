...
// --- HELPERS ---
// Normaliza el rol guardado en Firestore a una etiqueta legible.
// Evita mostrar valores crudos ("worker", "cliente", undefined) directamente en la UI.
const ROLE_LABELS = {
  cliente: "Cliente",
  client: "Cliente",
  worker: "Trabajador",
...
function getRoleLabel(rol) {
  ...
  const normalized = String(rol).toLowerCase().trim();
  return ROLE_LABELS[normalized] || rol.charAt(0).toUpperCase() + rol.slice(1);
}
...
export default function NexoraProfile() {
  ...
  useEffect(() => {
    ...
    const unsubTrabajos = onSnapshot(qTrabajos, (snap) =>
      setRawTrabajos(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
    );

    const qReseñas = query(
      collection(db, "reviews"),
      where("targetUserId", "==", user.uid),
    );
    const unsubReseñas = onSnapshot(qReseñas, (snap) =>
      setRawReseñas(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
    );
  ...
  // IMPORTANTE: la colección "solicitudes" también almacena las solicitudes de
  // verificación para convertirse en trabajador (tipo: "solicitud_trabajador",
  // ver worker/page.jsx). Esas NO son trabajos reales, así que se excluyen aquí
  // para que no aparezcan mezcladas como "trabajos completados" en el perfil.
  const trabajos = useMemo(
    () =>
      rawTrabajos
        .filter(
          (t) => t.estado === "Aprobado" && t.tipo !== "solicitud_trabajador",
        )
  ...
  // Rol legible, derivado una sola vez por render de userData.
  const roleLabel = useMemo(() => getRoleLabel(userData?.rol), [userData?.rol]);

  // Nombre a mostrar, con fallback en cascada:
  // 1) Firestore (userData) — fuente principal.
  // 2) AuthContext (user) — por si Firestore aún no tiene el campo pero el
  //    contexto lo recuperó de localStorage.
  // 3) Email — último recurso para que el header nunca quede en blanco.
  // Esto es un parche defensivo; el arreglo de raíz es asegurar que el
  // registro guarde first_name/last_name en Firestore para toda cuenta nueva.
  ...
  // Tasa de éxito: solo tiene sentido mostrarla si hay trabajos completados registrados.
  // Antes se mostraba "100%" fijo incluso sin datos, lo cual era engañoso.
  const tasaExito = useMemo(() => {
    // Igual que en "trabajos": se excluyen las solicitudes de verificación de
    // trabajador, para que la tasa refleje solo trabajos reales.
    const trabajosReales = rawTrabajos.filter(
      (t) => t.tipo !== "solicitud_trabajador",
    );
    if (trabajosReales.length === 0) return "—";
    const completados = trabajosReales.filter(
      (t) => t.estado === "Aprobado",
    ).length;
    return `${Math.round((completados / trabajosReales.length) * 100)}%`;
  }, [rawTrabajos]);
            ...
            <div className="space-y-8">
              <div className="bg-[#111118] rounded-[2.5rem] p-8 border border-white/5">
                ...
                <div className="space-y-6 text-sm font-bold">
                  <div className="flex justify-between text-slate-400 border-b border-white/3 pb-4">
                    <span>Ciudad</span>
              ...
              <div className="flex p-2 bg-[#111118] rounded-2xl border border-white/5 gap-2">
                {["Trabajos", "Reseñas", "Sobre mí", "Portfolio"].map((tab) => (
                  <button
                    ...
                    className={`flex-1 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${activeTab === tab ? "bg-[#6c63ff] text-white shadow-xl" : "text-slate-500 hover:text-white"}`}
                  >
                    {tab}
                  </button>
                ))}
                ...
                {activeTab === "Trabajos" && (
                  <div className="space-y-6">
                    {trabajos.length > 0 ? (
                      ...
                      <div className="bg-[#111118] rounded-[2.5rem] p-20 text-center border border-white/5 opacity-30 font-black uppercase tracking-[0.3em] text-[10px]">
                        Sin trabajos aún
                      </div>
                    )}
                  </div>
                ...
                {activeTab === "Portfolio" && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {portfolio.length > 0 ? (
                      ...
                      <div className="col-span-full bg-[#111118] rounded-[2.5rem] p-20 text-center border border-white/5 opacity-30 font-black uppercase tracking-[0.3em] text-[10px]">
                        Portfolio vacío
                      </div>
                    )}
                  </div>
              ...
              <div className="space-y-8">
                ...
                <div className="flex items-center justify-between p-6 bg-[#0A0A0F] rounded-2xl border border-white/5">
                  ...
                  <button
                    ...
                    className={`w-12 h-7 rounded-full relative transition-all ${editData.disponibilidad ? "bg-emerald-500" : "bg-slate-800"}`}
                  >
                    <div
                      className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${editData.disponibilidad ? "left-6" : "left-1"}`}
                    ></div>
                  </button>
                </div>
                ...
                <div className="flex flex-col items-center gap-6 pb-8 border-b border-white/3">
                  ...
                  <label className="bg-[#6c63ff] hover:bg-[#5a52d5] text-white px-6 py-3 rounded-2xl font-black uppercase tracking-widest text-[10px] cursor-pointer transition-all">
                    Cambiar Foto
                    <input
                      type="file"
                      accept="image/*"
                ...
                <div className="grid grid-cols-2 gap-6">
                  <input
                    ...
                    className="bg-[#0A0A0F] border border-white/10 rounded-2xl px-6 py-3 text-white font-bold"
                  />
                  <input
                    type="text"
                ...
                <input
                  ...
                  className="w-full bg-[#0A0A0F] border border-white/10 rounded-2xl px-6 py-3 text-white font-bold"
                />
                <input
                  type="tel"
                ...
                <textarea
                  ...
                  className="w-full bg-[#0A0A0F] border border-white/10 rounded-2xl px-6 py-3 text-white font-bold resize-none"
                />
                ...
                <div className="flex gap-4 pt-6">
                  ...
                  <button
                    ...
                    disabled={isSaving}
                    className="flex-1 bg-[#6c63ff] py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-[#6c63ff]/20"
                  >
            ...
            <div className="bg-[#111118] w-full max-w-lg rounded-[3rem] p-10 border border-white/5 relative shadow-2xl">
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  disabled={uploading}
                  className="flex-1 bg-[#6c63ff] py-4 rounded-2xl font-black uppercase tracking-widest text-[10px]"
                >
