...
// Si tu firebase/client.js ya exporta "auth", puedes borrar esta línea
// e importar { auth } directamente desde ahí en vez de { app }.
...
async function detectLocation() {
  try {
    ...
    if (data?.city)
      return `${data.city}, ${data.country_code || data.country_name || ""}`.trim();
...
function getWorkerStatusFromRequest(request, uid) {
  const match = (request.postulantes || []).find(
    (p) => getWorkerIdFromPostulante(p) === uid,
  );
  if (!match) return null;
  return typeof match === "object" ? match.estado || "postulado" : "postulado";
}

// ---- Validadores de campos (Perfil / Cuenta) ----
...
function sanitizePhone(value) {
  return value.replace(PHONE_REGEX, "").slice(0, LIMITS.phone);
}
...
function Toggle({ checked, onChange }) {
  return (
    ...
    >
      <div
        className={`absolute top-1 w-5 h-5 bg-white rounded-full transition-all ${checked ? "left-6" : "left-1"}`}
      />
    </button>
  );
}
  ...
  return (
    <div className="relative">
      <input
        ...
        className="w-full bg-[#0A0A0F] border border-white/10 rounded-2xl px-5 py-3.5 pr-12 text-white font-bold outline-none focus:border-[#6c63ff]"
      />
      <button
        type="button"
...
function SettingsModal({
  ...
  // ----- Apariencia (solo visual, no persiste todavía) -----
  const [settings, setSettings] = useState({
    theme: userData?.theme || "Oscuro",
  });
  ...
  // Requiere que en el login se cree un documento en la colección "sessions"
  // con { userId, device, location, lastActive, createdAt } y que se guarde
  // ese id de sesión en localStorage bajo "nexora_session_id" para marcar
  // cuál es la sesión actual.
  useEffect(() => {
    if (!user?.uid) return;
    const currentSessionId =
      typeof window !== "undefined"
        ? window.localStorage.getItem("nexora_session_id")
        : null;
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (
        ...
        showToast("Vuelve a iniciar sesión e inténtalo de nuevo", "error");
      } else {
  ...
  const confirmDeactivateAccount = async () => {
    ...
    try {
      ...
      showToast(
        "Cuenta desactivada. Tienes 30 días para reactivarla iniciando sesión.",
      );
      await signOut(auth);
      router.push("/login");
    } catch {
  ...
  // ----- Handlers de validación Perfil / Cuenta -----
  const handleUsernameChange = (e) =>
    setEditData({ ...editData, username: sanitizeUsername(e.target.value) });
  const handleFirstNameChange = (e) =>
    setEditData({ ...editData, first_name: sanitizeName(e.target.value) });
  const handleLastNameChange = (e) =>
    setEditData({ ...editData, last_name: sanitizeName(e.target.value) });
  const handleCityChange = (e) =>
    setEditData({ ...editData, city: sanitizeCity(e.target.value) });
  const handleWebsiteChange = (e) =>
    setEditData({
      ...editData,
      website: e.target.value.slice(0, LIMITS.website),
    });
  const handleAboutMeChange = (e) =>
    setEditData({
      ...editData,
      about_me: e.target.value.slice(0, LIMITS.about_me),
    });
  const handlePhoneChange = (e) =>
    setEditData({ ...editData, phone: sanitizePhone(e.target.value) });
  ...
  const Footer = ({ danger = false, hideSave = false }) => (
    <div className="flex gap-4 pt-6">
      <button
        onClick={onClose}
        className="flex-1 bg-white/5 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] border border-white/10"
      >
        Cancelar
      </button>
      {!danger && !hideSave && (
        <button
          ...
          className="flex-1 bg-[#6c63ff] py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-[#6c63ff]/20 disabled:opacity-50"
        >
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-xl flex items-center justify-center z-100 p-6">
      ...
      <div className="bg-[#111118] w-full max-w-5xl rounded-[3rem] border border-white/5 relative shadow-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-[240px_1fr] max-h-[90vh]">
          <aside className="bg-[#0A0A0F] border-r border-white/5 p-6 space-y-2 overflow-y-auto custom-scrollbar max-h-[90vh]">
            <h2 className="text-xl font-black mb-6 uppercase tracking-widest">
              Configuración
            </h2>
            {tabs.map((tab) => (
              <button
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-slate-500 font-bold mt-1">
                    Tu información pública en Nexora
                  </p>
                </div>
                ...
                <div className="flex flex-col items-center gap-4">
                  <label className="w-24 h-24 rounded-full overflow-hidden border-4 border-[#6c63ff] cursor-pointer group relative">
                    {userData.photoURL ? (
                      ...
                      <div className="w-full h-full bg-[#6c63ff] flex items-center justify-center text-3xl font-black">
                        {avatarInitials}
                      </div>
                    )}
                    <input
                ...
                <Field
                  ...
                  counter={`${(editData.username || "").length}/${LIMITS.username}`}
                >
                  <Input
                    value={editData.username || username}
                    onChange={handleUsernameChange}
                    placeholder="usuario_nexora"
                  />
                </Field>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <Field
                    label="Nombre"
                    counter={`${(editData.first_name || "").length}/${LIMITS.first_name}`}
                  >
                    <Input
                  ...
                  <Field
                    label="Apellido"
                    counter={`${(editData.last_name || "").length}/${LIMITS.last_name}`}
                  >
                    <Input
                ...
                <Field
                  ...
                  counter={`${(editData.about_me || "").length}/${LIMITS.about_me}`}
                >
                  <textarea
                    rows="3"
                    maxLength={LIMITS.about_me}
                ...
                <Field
                  label="Ubicación"
                  counter={`${(editData.city || "").length}/${LIMITS.city}`}
                >
                  <Input value={editData.city} onChange={handleCityChange} />
                </Field>

                <Field
                  label="Sitio web"
                  hint={websiteError ? "URL inválida" : undefined}
                  counter={`${(editData.website || "").length}/${LIMITS.website}`}
                >
                  <Input
                    value={editData.website || ""}
                    onChange={handleWebsiteChange}
                    placeholder="https://juanperez.dev"
                    className={websiteError ? "border-red-500/50" : ""}
                  />
                </Field>
            ...
            {section === "Cuenta" && (
              <div className="space-y-8">
                ...
                <Field
                  ...
                  hint="Para notificaciones y recuperación"
                >
                  <Input
                ...
                <Field
                  ...
                  counter={`${(editData.phone || "").length}/${LIMITS.phone}`}
                >
                  <Input
                    type="tel"
                    inputMode="numeric"
            ...
            {section === "Seguridad" && (
              <div className="space-y-8">
                ...
                <button
                  ...
                  className="w-full bg-[#6c63ff] py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-[#6c63ff]/20 disabled:opacity-50"
                >
                  {isChangingPassword
                    ? "Actualizando..."
                    : "Cambiar contraseña"}
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between bg-[#0A0A0F] border border-white/5 rounded-2xl p-5">
                    ...
                    <Toggle
                      ...
                      onChange={(v) =>
                        handleToggleSecuritySetting("twoFactor", v)
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between bg-[#0A0A0F] border border-white/5 rounded-2xl p-5">
                    <div>
                      ...
                      <p className="text-slate-500 text-xs font-bold mt-1">
                        Notificar al iniciar sesión desde nuevo dispositivo
                      </p>
                    </div>
                    <Toggle
                      checked={securitySettings.sessionAlerts}
                      onChange={(v) =>
                        handleToggleSecuritySetting("sessionAlerts", v)
                      }
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  {sessions.length > 0 ? (
                      ...
                      >
                        <div>
                          <p className="text-white font-bold">
                            {s.device || "Dispositivo desconocido"}
                          </p>
                          <p className="text-slate-500 text-xs font-bold">
                            {s.location || "Ubicación desconocida"} ·{" "}
                            {toDateTimeLabel(s.lastActive)}
                          </p>
                        </div>
                        {s.isCurrent ? (
                    ...
                    <p className="text-slate-600 text-xs font-bold py-4">
            ...
            {section === "Apariencia" && (
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-slate-500 font-bold mt-1">
                    Personaliza la interfaz según tus gustos
                  </p>
                </div>
                      ...
                      <button
                        ...
                        className={`text-left p-5 rounded-2xl border transition-all ${
                          settings.theme === theme
                            ...
                            : "bg-[#0A0A0F] border-white/5 text-slate-400"
                        }`}
                      >
                ...
                <div className="bg-red-500/5 border border-red-500/20 rounded-3xl p-6 space-y-4">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-slate-500 text-sm font-bold mt-1">
                        Tu cuenta quedará pausada. Si no inicias sesión en 30
                        días, se eliminará permanentemente.
                      </p>
                    </div>
                    <button
                      onClick={() => setShowDeactivateConfirm(true)}
                      disabled={isDeactivating}
                      className="bg-white/5 border border-white/10 text-white px-5 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest disabled:opacity-50"
                    >
                ...
                <div className="bg-red-500/10 border border-red-500/30 rounded-3xl p-6 space-y-5">
                  <div>
                    ...
                    <p className="text-slate-400 text-sm font-bold mt-1">
                      Esta acción eliminará todos tus datos, proyectos y
                  ...
                  <Field
                    label="Contraseña"
                    hint="Necesaria para confirmar tu identidad"
                  >
                    <PasswordInput
                      value={dangerPassword}
                      onChange={(e) => setDangerPassword(e.target.value)}
                      show={showDangerPassword}
                      onToggleShow={() => setShowDangerPassword((v) => !v)}
                  ...
                  <button
                    ...
                    disabled={dangerText !== "ELIMINAR" || isDeleting}
          ...
          <div className="bg-[#111118] w-full max-w-md rounded-[2.5rem] p-10 border border-amber-500/20 shadow-2xl text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-slate-400 font-bold text-sm mt-3 leading-relaxed">
                Tu perfil dejará de estar visible y se cerrará tu sesión ahora
                mismo. Si no vuelves a iniciar sesión dentro de{" "}
                <span className="text-amber-400">30 días</span>, tu cuenta y
            ...
            <div className="flex gap-4">
              <button
                ...
                disabled={isDeactivating}
                className="flex-1 bg-white/5 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] border border-white/10 disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={isDeactivating}
                className="flex-1 bg-amber-500 hover:bg-amber-600 text-black py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] disabled:opacity-50"
              >
...
function NexoraProfileContent() {
  ...
  // Registra la sesión actual en Firestore y la mantiene viva con un
  // heartbeat cada 5 minutos. Esto hace que "Sesiones activas" en
  // Seguridad muestre datos reales en vez de quedarse en 0.
  // Recomendado: mueve este mismo bloque a tu AuthContext o layout raíz
  // para que la sesión se registre apenas el usuario inicia sesión en
  // cualquier parte de la app, no solo al visitar /profile.
  useEffect(() => {
    if (!user?.uid) return;
    let cancelled = false;
    ...
    const registerSession = async () => {
      try {
        ...
        // silencioso: no bloquea el uso del perfil si falla el registro
      }
    };
  ...
  useEffect(() => {
    ...
    const unsubAll = onSnapshot(
      ...
      (snap) =>
        setRawAllRequests(snap.docs.map((d) => ({ id: d.id, ...d.data() }))),
      () => setRawAllRequests([]),
    );
  ...
  const displayName = useMemo(() => {
    ...
    return titleCaseName(
      fromFirestore || userData?.displayName || user?.email || "Usuario",
    );
  }, [userData, user]);
  ...
  const serviciosCliente = useMemo(
    () =>
      rawOwnRequests
        ...
        .sort(
          (a, b) =>
            ((b.creadoEn || b.fecha)?.seconds || 0) -
            ((a.creadoEn || a.fecha)?.seconds || 0),
        ),
    [rawOwnRequests],
  );
  ...
  const trabajosTrabajador = useMemo(
    () =>
      rawAllRequests
        ...
        .filter((t) =>
          (t.postulantes || []).some(
            (p) => getWorkerIdFromPostulante(p) === user?.uid,
          ),
        )
        .sort(
          (a, b) =>
  ...
  const mainItems = isAdmin
    ...
    : isWorker
      ? trabajosTrabajador
      : serviciosCliente;
  const mainLabel = isAdmin
    ? "Solicitudes"
    : isWorker
      ? "Trabajos"
      : "Servicios";
  const emptyMainLabel = isAdmin
    ? "Sin solicitudes aún"
    : isWorker
      ? "Sin trabajos aún"
      : "Sin servicios aún";
  ...
  const tasaExito = useMemo(() => {
    ...
    const completados = base.filter((t) =>
      ["Aprobado", "finalizado", "completado"].includes(t.estado),
    ).length;
  ...
  const stats = useMemo(() => {
    if (isAdmin) {
      return [
        ...
        {
          value: rawAllRequests.filter((r) => r.tipo === "solicitud_trabajador")
            .length,
  ...
  const handleSaveSettings = async () => {
    if (!user?.uid) return;
    if (editData.website && !isValidWebsite(editData.website)) {
      showToast("El sitio web no es una URL válida", "error");
      return;
    }
    try {
      setIsSaving(true);
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start gap-10">
            ...
            <div className="flex-1 text-center md:text-left space-y-4">
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-6 text-sm font-bold text-slate-500 uppercase tracking-widest">
                ...
                {!isAdmin && (
                  <span
                    className={`flex items-center gap-2 ${userData.disponibilidad ? "text-emerald-400" : "text-red-400"}`}
                  >
                    <div
                      className={`w-2 h-2 rounded-full ${userData.disponibilidad ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`}
                    />
                ...
                <span className="flex items-center gap-2 text-amber-400">
                  <Icons.Star fill="currentColor" /> {reputacion.score} (
                  {reputacion.count} reseñas)
                </span>
              </div>
              ...
              <p className="text-slate-400 text-lg leading-relaxed max-w-2xl font-medium italic">
                ...
                {userData.about_me ||
                  (isWorker
                    ? "Trabajador profesional en Nexora."
                    : isAdmin
                      ? "Administrador de la plataforma Nexora."
                      : "Cliente en Nexora.")}
                "
              </p>
            </div>
            ...
            <div className="bg-[#111118] rounded-[2.5rem] p-8 border border-white/5">
              ...
              <div className="space-y-6 text-sm font-bold">
                ...
                <div className="flex justify-between text-slate-400">
                  <span>Rol</span>
                  <span className="text-[#6c63ff] uppercase font-black">
                    {roleLabel}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3 text-xs font-bold text-slate-400">
                {[
                  ...
                  isWorker
                    ...
                    : isAdmin
                      ? "Permisos administrativos activos"
                      : "Cuenta lista para publicar servicios",
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex-1 min-w-fit px-4 py-3 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                    activeTab === tab
                      ? "bg-[#6c63ff] text-white shadow-xl"
            ...
            <div className="min-h-75">
              {(activeTab === "Trabajos" ||
                ...
                <div className="space-y-6">
                  {mainItems.length > 0 ? (
                    mainItems.map((item) => {
                      const workerStatus = isWorker
                        ? getWorkerStatusFromRequest(item, user?.uid)
                        : null;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="text-[10px] font-black text-slate-600 uppercase tracking-widest flex flex-wrap items-center gap-4">
                            <span className="flex items-center gap-2">
                              <Icons.Calendar />{" "}
                              {toDateLabel(getRequestDate(item))}
                            </span>
                ...
                <div className="space-y-6">
                  {resenas.length > 0 ? (
                    resenas.map((r) => (
                      <div
                        key={r.id}
                        className="bg-[#111118] rounded-[2.5rem] p-8 border border-white/5"
                      >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {portfolio.length > 0 ? (
                    portfolio.map((p) => (
                      <div
                        key={p.id}
                        className="bg-[#111118] rounded-[2.5rem] overflow-hidden border border-white/5 group hover:border-[#6c63ff]/30 transition-all"
                      >
          ...
          <div className="bg-[#111118] w-full max-w-lg rounded-[3rem] p-10 border border-white/5 relative shadow-2xl">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 bg-white/5 py-4 rounded-2xl font-black uppercase tracking-widest text-[10px]"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={uploading}
                className="flex-1 bg-[#6c63ff] py-4 rounded-2xl font-black uppercase tracking-widest text-[10px] disabled:opacity-50"
              >
