    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/90 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-[#111118] border border-white/10 w-full max-w-5xl max-h-[95vh] rounded-[3rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
          ...
          <button onClick={onClose} className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:bg-white/10 hover:text-white transition-all">
            <Icons.X />
          </button>
        </div>
              ...
              <section>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex gap-4">
                    <button onClick={() => setShowPdf(!showPdf)} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-white transition-colors">
                  ...
                  <div className="space-y-4">
                    {showPdf ? (
                      ...
                      <div className="bg-[#0A0A0F] border border-white/5 rounded-2xl p-6 flex items-center justify-between group">
                        ...
                        <button onClick={() => setShowPdf(true)} className="bg-white/5 text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">Abrir Visor</button>
                      </div>
                    )}
                  </div>
                ) : (
                  <div className="bg-[#0A0A0F] border border-white/5 rounded-2xl p-10 text-center text-slate-600 text-[10px] font-black uppercase tracking-widest">No se adjuntó CV</div>
                )}
              </section>
            </div>
                  ...
                  {solicitud.imagenesUrls && solicitud.imagenesUrls.length > 0 ? (
                    <div className="grid grid-cols-2 gap-4">
                      {solicitud.imagenesUrls.map((img, i) => (
                        <div key={i} onClick={() => openImage(img, i)} className="relative aspect-square rounded-2xl overflow-hidden border border-white/10 hover:border-[#6c63ff]/50 transition-all group cursor-pointer shadow-lg">
                          ...
                          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                            <div className="text-center">
                              <Icons.Search />
                              <span className="text-white text-[8px] font-black uppercase tracking-widest block mt-1">Ampliar</span>
                            </div>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="h-full flex items-center justify-center text-slate-600 text-[10px] font-black uppercase tracking-widest">Sin imágenes adjuntas</div>
                  )}
                </div>
              </section>
            </div>
        ...
        <div className="p-8 border-t border-white/5 bg-white/[0.02] flex gap-4">
          ...
          <button onClick={() => { onReject(solicitud); onClose(); }} className="flex-1 bg-red-500/10 text-red-500 border border-red-500/20 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2">
...
export default function AdminPage() {
  ...
  const registrarActividad = async (mensaje, tipo = "info") => {
    try {
      await addDoc(collection(db, "actividad"), {
        mensaje, tipo, timestamp: serverTimestamp(),
  ...
  const handleDelete = async (userId, userName) => {
    ...
    try {
      ...
      await registrarActividad(`Eliminación: Usuario ${userName}`, "delete");
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-20 space-y-12">
        <div className="relative z-10">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <p className="text-slate-500 text-[11px] uppercase tracking-[0.3em] font-black">Control de Sistema en Tiempo Real</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            { label: "Usuarios Totales", value: stats.totalUsers, sub: `${stats.clientes} Clientes / ${stats.trabajadores} Trab.`, color: "border-blue-500/20 bg-blue-500/5", icon: <Icons.Users /> },
            { label: "Solicitudes", value: stats.totalSolicitudes, sub: "Historial completo", color: "border-purple-500/20 bg-purple-500/5", icon: <Icons.FileText /> },
            { label: "Pendientes", value: stats.pendientes, sub: "Por verificar", color: "border-amber-500/20 bg-amber-500/5", icon: <Icons.Clock /> },
          ].map((item, i) => (
            <div key={i} className={`border ${item.color} p-10 rounded-[2.5rem] transition-all duration-500 hover:scale-[1.03] relative overflow-hidden group`}>
              ...
              <h3 className="text-slate-500 text-[10px] font-black uppercase tracking-widest">{item.label}</h3>
              <p className="text-6xl font-black text-white mt-4 tracking-tighter">{item.value}</p>
              <p className="text-xs text-slate-500 mt-6 font-bold">{item.sub}</p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-12">
            ...
            <section className="bg-[#111118] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
              <div className="p-10 border-b border-white/5 flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-black text-white">Solicitudes de Verificación</h2>
                  <p className="text-[10px] text-amber-500 uppercase tracking-[0.2em] mt-2 font-bold">Clientes que desean ser trabajadores</p>
                </div>
                <div className="w-10 h-10 rounded-2xl bg-amber-500/10 flex items-center justify-center text-amber-500"><Icons.Clock /></div>
              </div>
              <div className="divide-y divide-white/3">
                {solicitudes.filter((s) => s.estado === "pendiente").length === 0 ? (
                  <div className="p-20 text-center text-slate-600 font-bold uppercase tracking-widest text-[10px]">No hay solicitudes pendientes</div>
                ) : (
                  solicitudes.filter((s) => s.estado === "pendiente").map((s) => (
                    <div key={s.id} className="p-10 flex flex-col md:flex-row md:items-center justify-between hover:bg-white/[0.01] transition-all gap-4 group/item">
                      <div className="flex items-center gap-6 cursor-pointer" onClick={() => setSelectedSolicitud(s)}>
                        ...
                        <div className="space-y-1">
                          <p className="text-lg font-black text-white group-hover/item:text-[#6c63ff] transition-colors">{s.nombre}</p>
                          <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">{s.profesion} • {s.experiencia} años exp.</p>
                        </div>
                      </div>
                      <div className="flex gap-4 shrink-0">
                        <button onClick={() => setSelectedSolicitud(s)} className="bg-white/5 text-white border border-white/10 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all flex items-center gap-2"><Icons.Eye /> Detalles</button>
                        <button onClick={() => handleApproveWorker(s)} className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all flex items-center gap-2"><Icons.Check /> Aprobar</button>
                      </div>
                    </div>
                  ))
                )}
            ...
            <section className="bg-[#111118] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
              <div className="p-10 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <h2 className="text-2xl font-black text-white flex items-center gap-4"><Icons.Users /> Gestión de Usuarios</h2>
                <div className="relative w-full md:w-80">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500"><Icons.Search /></div>
                  <input type="text" placeholder="Buscar..." className="bg-[#0A0A0F] border border-white/10 rounded-2xl pl-12 pr-8 py-4 text-sm focus:ring-2 focus:ring-[#6c63ff] outline-none w-full text-white" value={search} onChange={(e) => setSearch(e.target.value)} />
                </div>
              </div>
                      ...
                      <tr key={u.id} className="hover:bg-white/[0.01] transition-colors group">
                        <td className="px-10 py-8">
                          <div className="flex items-center gap-5">
                            ...
                            <div><p className="font-bold text-white">{u.first_name} {u.last_name}</p><p className="text-xs text-slate-600">{u.email}</p></div>
                          </div>
                        </td>
                        <td className="px-10 py-8"><span className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border ${ROLE_COLORS[u.rol]}`}>{ROLE_LABELS[u.rol]}</span></td>
                        <td className="px-10 py-8 text-right">
                          <div className="flex items-center justify-end gap-4 opacity-0 group-hover:opacity-100 transition-all">
                            <select className="bg-[#0A0A0F] border border-white/10 rounded-xl text-[10px] px-4 py-2 text-white font-black cursor-pointer outline-none focus:border-[#6c63ff]" value={u.rol} onChange={(e) => handleRoleChange(u.id, u.first_name, e.target.value)}>
                              <option value="cliente">Cliente</option><option value="trabajador">Trabajador</option><option value="admin">Admin</option>
                            </select>
                            <button onClick={() => handleDelete(u.id, u.first_name)} className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Icons.Trash /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          <div className="lg:col-span-4">
            <section className="bg-[#111118] border border-white/5 rounded-[3rem] p-10 sticky top-32 shadow-2xl h-fit max-h-[70vh] flex flex-col">
              <h2 className="text-2xl font-black text-white mb-6 flex items-center gap-3"><Icons.Refresh /> Actividad Live</h2>
              <div className="flex-1 overflow-y-auto pr-4 space-y-6 relative before:absolute before:left-4 before:top-0 before:bottom-0 before:w-0.5 before:bg-white/3 custom-scrollbar">
                {actividades.length === 0 ? (
                  <div className="text-center py-10 opacity-30 text-[10px] font-black uppercase tracking-widest">Esperando eventos...</div>
                ) : (
                  actividades.map((act) => (
                    <div key={act.id} className="relative pl-12 group">
                      <div className={`absolute left-0 top-1 w-8 h-8 rounded-lg border-4 border-[#111118] flex items-center justify-center shadow-lg z-10 ${act.tipo === "delete" ? "bg-red-500 text-white" : act.tipo === "update" ? "bg-blue-500 text-white" : "bg-[#6c63ff] text-white"}`}>
                        {act.tipo === "delete" ? <Icons.Trash /> : act.tipo === "update" ? <Icons.Refresh /> : <Icons.Check />}
                      </div>
                      <p className="text-xs font-bold text-slate-300 leading-relaxed break-words">{act.mensaje}</p>
                      <p className="text-[9px] text-slate-600 mt-1 font-black uppercase tracking-widest">{act.fecha}</p>
                    </div>
                  ))
                )}
              </div>
