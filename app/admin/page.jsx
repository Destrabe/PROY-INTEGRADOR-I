...
function SolicitudModal({ solicitud, onClose, onApprove, onReject, users }) {
  ...
  // Función para abrir la imagen en una nueva pestaña con un título
...
export default function AdminPage() {
  ...
  const handleApproveWorker = async (solicitud) => {
    try {
      ...
      registrarActividad(`Solicitud de ${solicitud.nombre} APROBADA`, "update");
      alert(`${solicitud.nombre} ahora es Trabajador.`);
    } catch (error) {
  ...
  const handleDelete = async (userId, userName) => {
    ...
    try {
      ...
      registrarActividad(`Usuario ${userName} eliminado`, "delete");
    } catch (error) {
      ...
      <main className="max-w-7xl mx-auto px-6 pt-28 pb-20 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div
              key={i}
              className={`border ${item.color} p-10 rounded-[2.5rem] transition-all duration-500 hover:scale-[1.03]`}
            >
                      ...
                      >
                        ...
                        <div className="flex gap-4 shrink-0">
                          ...
                          <button
                            ...
                            className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-500 hover:text-white transition-all"
                          >
                            Aprobar
                          </button>
                        </div>
            ...
            <section className="bg-[#111118] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
              <div className="p-10 border-b border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-8">
                <div>
                  ...
                  <p className="text-[10px] text-slate-600 uppercase tracking-[0.2em] mt-2 font-bold">
                    Directorio central
                  </p>
                </div>
                <input
                  type="text"
                  placeholder="Buscar..."
                  className="bg-[#0A0A0F] border border-white/10 rounded-2xl px-8 py-4 text-sm focus:ring-2 focus:ring-[#6c63ff] outline-none w-full md:w-80 text-white"
                  value={search}
                ...
                {actividades.length === 0 ? (
                  ...
                  actividades.map((act) => (
                    <div key={act.id} className="relative pl-14 group">
                      ...
                      <p className="text-[10px] text-slate-600 mt-2 font-black uppercase tracking-widest">
                        {act.fecha}
                      </p>
                    </div>
                  ))
